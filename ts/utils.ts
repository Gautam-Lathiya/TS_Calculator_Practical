...
export const MathUtils = {
    ...
    countParentheses(expr: string): { open: number, close: number } {
    ...
    calculateFactorial(n: number): number {
      ...
      if (num > 200) throw new Error("Number too large for factorial");
      if (num === 0) return 1;
    ...
    calculateModulo(a: number, b: number): number {
      const num1 = parseFloat(a.toString());
      const num2 = parseFloat(b.toString());
    ...
    calculatePowerOf10(exponent: number): number {
      const num = parseFloat(exponent.toString());
    ...
    calculateTrig(operation: string, value: number): number {
